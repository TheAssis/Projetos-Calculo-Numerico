function [a] = aproximacaoTaylor(t0, numTermos, t)
    ...
    w0 = 5.3748377; //velocidade angular
    r = [528.6837 -5.58392 -196.69973]; //valores dos coeficientes calculados anteriormente por MMQ
    ...
    for i = 2:1:numTermos//começamos com o segundo termo pois o primeiro foi calculado acima.
        ...
        if modulo(i,4) == 2//primeira derivada vai aqui
            a = a + ( -(w0^(i-1))*r(2)*sin(w0*t0) + (w0^(i-1))*r(3)*cos(w0*t0) )*(t-t0)^(i-1) / fatorial(i-1); 
        elseif modulo(i,4) == 3//segunda derivada vai aqui
            a = a + ( -(w0^(i-1))*r(2)*cos(w0*t0) -(w0^(i-1))*r(3)*sin(w0*t0) )*(t-t0)^(i-1) / fatorial(i-1);
